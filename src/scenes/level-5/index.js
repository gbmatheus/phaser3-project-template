...
export default class MyGame extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON("map_5", mapJson);
  ...
  create() {
    this.map = this.make.tilemap({ key: "map_5" });
    ...
    this.physics.add.collider(this.player, this.objectCollider,  (obj1, obj2) => {
      ...
      if(obj2.properties.winner)
        this.game.events.emit(EventName.gameEnd, { gameStatus: gameStatus.win, level: 5 })
        // this.game.events.emit(EventName.gameEnd, gameStatus.win)
      if(obj2.properties.killer)
        this.game.events.emit(EventName.gameEnd, { gameStatus: gameStatus.lose, level: 5 })
